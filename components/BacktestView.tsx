        ...
        <div className="flex-1 space-y-6 md:space-y-8">
          ...
          <div className="space-y-4 md:space-y-6 max-w-4xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FundPicker label="标的 A" code={props.fundA.code} name={props.fundA.name} onUpdate={props.setFundA} placeholder="011861" />
              <FundPicker label="标的 B" code={props.fundB.code} name={props.fundB.name} onUpdate={props.setFundB} placeholder="020603" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="w-full lg:w-96 flex flex-col gap-6">
          <div className="bg-white p-5 md:p-7 rounded-3xl shadow-xl border border-slate-100">
            ...
            <div className="space-y-5 md:space-y-6">
              <div>
                <div className="flex justify-between mb-2 text-[10px] font-black uppercase text-slate-400"><span>动量回顾周期</span><span>{props.momentumN}天</span></div>
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">最短持仓保护</label>
                <div className="grid grid-cols-3 gap-2">
                  {[7, 14, 30].map(d => (
                    <button key={d} disabled={props.isParamLocked} onClick={() => props.setMinHoldDays(d)} className={`py-2 text-[10px] font-black rounded-xl border transition-all ${props.minHoldDays === d ? 'bg-slate-900 text-white border-slate-900 shadow-md' : 'bg-white text-slate-400 border-slate-100 hover:bg-slate-50'}`}>{d}天</button>
                  ))}
                </div>
              </div>
          ...
          <div className="bg-slate-50 p-5 md:p-6 rounded-3xl border-2 border-dashed border-slate-200">
            <h4 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest flex items-center justify-between">
              <span>资金追加模拟</span>
              <span className="bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded text-[8px]">SIMULATE</span>
            </h4>
            <div className="flex flex-col gap-2 mb-4">
              <div className="flex gap-2">
                <input type="date" value={newInj.date} onChange={e => setNewInj({...newInj, date: e.target.value})} className="flex-1 text-[11px] border rounded-lg px-2 py-2 font-bold outline-none bg-white" />
                <input type="number" value={newInj.amount || ''} placeholder="金额" onChange={e => setNewInj({...newInj, amount: Number(e.target.value)})} className="w-24 text-[11px] border rounded-lg px-2 py-2 font-bold outline-none bg-white" />
              </div>
              <button onClick={() => {props.addInjection(newInj); setNewInj({date:'', amount:0});}} className="bg-indigo-600 text-white py-2 rounded-lg text-xs font-black hover:bg-indigo-700 transition-colors w-full flex items-center justify-center gap-2">
                <i className="fa-solid fa-plus"></i> 添加模拟资金
              </button>
            </div>
            <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
              {props.injections.map(inj => (
                <div key={inj.id} className="bg-white px-3 py-1.5 rounded-full border shadow-sm flex items-center gap-2 group">
                  <span className="text-[10px] font-black font-mono">¥{inj.amount}</span>
                  <span className="text-[9px] text-slate-400">{inj.date.slice(5)}</span>
                  <button onClick={() => props.removeInjection(inj.id)} className="text-slate-300 group-hover:text-rose-500 transition-colors"><i className="fa-solid fa-circle-xmark"></i></button>
                </div>
              ))}
            </div>
          </div>
      ...
      {props.result ? (
        <div className="space-y-8 md:space-y-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
            ...
            <MetricsCard label="回撤修复" value={`${props.result.metrics.maxDrawdownDuration} 天`} subValue={`回撤: ${(props.result.metrics.maxDrawdown*100).toFixed(2)}%`} icon="fa-clock-rotate-left" colorClass="bg-rose-600" />
          </div>
          ...
          <div className="bg-white rounded-2xl md:rounded-[2rem] border overflow-hidden shadow-sm">
            <div className="px-5 md:px-8 py-4 md:py-6 bg-slate-50/50 border-b flex justify-between items-center">
              <h3 className="font-black text-slate-800 text-sm md:text-base">策略流水</h3>
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y">{props.result.trades.slice().reverse().map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 md:px-8 py-4 text-center"><span className={`px-2 py-0.5 rounded text-[9px] md:text-[10px] font-black ${t.reason.includes('追加') ? 'bg-indigo-100 text-indigo-700' : t.type === 'BUY' ? 'bg-rose-50 text-rose-600' : 'bg-emerald-50 text-emerald-600'}`}>{t.reason.includes('追加') ? '追加' : t.type === 'BUY' ? '买入' : '卖出'}</span></td>
                    <td className="px-5 md:px-8 py-4 text-right font-mono font-bold">¥{t.totalAmount.toFixed(0)}</td>
                    <td className="px-5 md:px-8 py-4 text-right font-mono font-black text-amber-600">¥{t.cost.toFixed(2)}</td>
