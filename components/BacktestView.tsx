      ...
      <header className="mb-8 md:mb-10 flex flex-col lg:flex-row lg:items-start justify-between border-b pb-8 md:pb-10 gap-8">
        <div className="flex-1 space-y-6 md:space-y-8">
          <div className="flex items-center gap-4">
             ...
             <span className="bg-emerald-100 text-emerald-700 text-[10px] px-2 py-1 rounded-full font-black border border-emerald-200">免滑点/C类持有模型</span>
          </div>
        ...
        <div className="w-full lg:w-96 flex flex-col gap-6">
          <div className="bg-white p-5 md:p-7 rounded-3xl shadow-xl border border-slate-100">
            ...
            <div className="space-y-5 md:space-y-6">
              ...
              <div>
                ...
                <p className="text-[9px] text-slate-400 mt-2 italic font-bold leading-tight">* C类持有不满7天赎回将面临1.5%高额费率。建议设为7天。</p>
              </div>
          ...
          <div className="bg-slate-50 p-5 md:p-6 rounded-3xl border-2 border-dashed border-slate-200">
             <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">费率模型 (C 类)</span>
                <i className="fa-solid fa-circle-info text-slate-300"></i>
             </div>
             <ul className="text-[10px] font-bold text-slate-500 space-y-2">
                <li className="flex justify-between"><span>申购费</span><span className="text-emerald-600">0.00% (FREE)</span></li>
                <li className="flex justify-between"><span>滑点设定</span><span className="text-slate-700">0.00% (NO SLIP)</span></li>
                <li className="flex justify-between"><span>赎回费 (&lt; 7天)</span><span className="text-rose-600">1.50% (PUNISH)</span></li>
                <li className="flex justify-between"><span>赎回费 (&ge; 7天)</span><span className="text-emerald-600">0.00% (FREE)</span></li>
             </ul>
          </div>
        </div>
      </header>
      ...
      {props.result ? (
        <div className="space-y-8 md:space-y-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
            ...
            <MetricsCard label="最大回撤" value={`${(props.result.metrics.maxDrawdown*100).toFixed(2)}%`} subValue={`回撤周期: ${props.result.metrics.maxDrawdownDuration} 交易日`} icon="fa-arrow-down-wide-short" colorClass="bg-rose-600" />
            <MetricsCard label="年化收益" value={`${(props.result.metrics.annualizedReturn*100).toFixed(2)}%`} subValue={`成本损耗: ¥${props.result.metrics.totalCosts.toFixed(0)}`} icon="fa-bolt" colorClass="bg-amber-600" />
          </div>
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y">{props.result.trades.slice().reverse().map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className={`px-5 md:px-8 py-4 text-right font-mono font-black ${t.cost > 0 ? 'text-rose-600' : 'text-emerald-500'}`}>
                      {t.cost > 0 ? `¥${t.cost.toFixed(2)}` : '0.00'}
                    </td>
